...
def step0_collect():
    ...
    wb.save("1_Book1_raw.xlsx")
    print(f"  📊 Saved {len(all_rows)} rows to 1_Book1_raw.xlsx")
    return True
...
def step1_swap():
    ...
    df = pd.read_excel("1_Book1_raw.xlsx")
    ...
    df.to_excel("2_Book1_swapped.xlsx", index=False)
    print("  ✅ Saved 2_Book1_swapped.xlsx")
...
def step2_clean():
    ...
    df = pd.read_excel("2_Book1_swapped.xlsx")
    ...
    df.to_excel("3_Book1_dropzero.xlsx", index=False)
    print(f"\n  ✅ Saved 3_Book1_dropzero.xlsx")
...
def step3_drop_correlated():
    ...
    df = pd.read_excel("3_Book1_dropzero.xlsx")
    ...
    columns_to_drop = [
        ...
        'tot_hbond_pm', 'tot_seq_recovery', 'dslf_fa13', 'ligand_is_touching_A',
        'user_tag'
    ]
    ...
    df.to_excel("4_Book1_cleaned.xlsx", index=False)
...
def step4_recheck():
    ...
    df = pd.read_excel("4_Book1_cleaned.xlsx")
    numeric_df = df.select_dtypes(include='number')
...
def step5_normalize():
    ...
    df.to_excel("5_Book1_normalized.xlsx", index=False)
    print("  ✅ Saved 5_Book1_normalized.xlsx")
...
def step6_recheck():
    ...
    df = pd.read_excel("5_Book1_normalized.xlsx")
    numeric_df = df.select_dtypes(include='number')
...
def step7_drop_and_recheck():
    ...
    df.drop(columns=dropped, inplace=True)
    df.to_excel("6_Book1_normalized_cleaned.xlsx", index=False)
    ...
    if not_found:
        print(f"  ⚠️  Not found (already removed): {not_found}")
    print(f"\n  ✅ Saved 6_Book1_normalized_cleaned.xlsx")
...
def step8_aggregate():
    ...
    df = pd.read_excel("6_Book1_normalized_cleaned.xlsx")
    df['GH3'] = df['GH3'].astype(str)
    df['oligo'] = df['oligo'].astype(str)
    agg_df = df.groupby(['GH3', 'oligo']).mean(numeric_only=True).reset_index()
    agg_df.to_csv("7_Book1_aggregated.csv", index=False)
    print(f"  ✅ Saved 7_Book1_aggregated.csv ({len(agg_df)} rows)")
...
def step9_label_active():
    ...
    df = pd.read_csv("7_Book1_aggregated.csv")
    ...
    df.to_csv("8_Book1_final.csv", index=False)
    print(f"\n  💾 Saved 8_Book1_final.csv")
