#!/usr/bin/env python3
"""
1d_train_randomforest.py
------------------------
- Reads Book1_aggregated_with_split.csv (created by 1a_train_logreg.py)
- Ranks features using RandomForest native feature importances
...
MODEL_OUT       = 'model_randomforest.joblib'
FEATURES_OUT    = 'features_randomforest.txt'
THRESHOLD_OUT   = 'threshold_randomforest.txt'
METRICS_OUT     = 'metrics_randomforest.json'
EXCEL_OUT       = 'metrics_randomforest.xlsx'
PLOT_OUT        = 'feature_importance_randomforest.png'
...
# ============================================================
# FEATURE RANKING via RandomForest importances
# ============================================================
print("\n🔎 Ranking features with RandomForest feature importances...")

smote = SMOTE(random_state=RANDOM_STATE)
X_train_res, y_train_res = smote.fit_resample(X_train, y_train)

ranking_clf = RandomForestClassifier(
    n_estimators=1500,
...
while True:
    try:
        chosen_n = int(input(f"\n🔢 How many features for the final RandomForest model? (1–{len(ordered_features)}): "))
...
# TRAIN FINAL MODEL
# ============================================================
print(f"\n🏋️  Training final RandomForest model with {chosen_n} features...")
...
plt.title(f'RandomForest Feature Importances (n={chosen_n})')
plt.tight_layout()
plt.savefig(PLOT_OUT, dpi=300)
plt.close()
print(f"  ✅ Feature importance plot saved: {PLOT_OUT}")

print(f"\n🎉 Done! All 4 models trained. Run 2_compare_models.py next.")
